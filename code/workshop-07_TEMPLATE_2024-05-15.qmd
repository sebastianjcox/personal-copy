...
The .qmd file is under the 'code' folder and 'abalone_growth.csv' is in the
'data' folder.

Note the file path within the `read_csv()` function. How is this different from how we've stored and read data before?  

read_csv uses the here() operator to denote the files and folders found in the
working repository.
...
All three of the operators give information about the abalone data.
str() tells you the columns in the data frame and some of their values
glimpse() tells you the number of rows and columns in the data frame, and lists
all of the values for each column along with their names
class() tells you that it is a table
